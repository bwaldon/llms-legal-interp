...
class MetaLinguisticJudgement:
    ...
    def probs(self, prompts: List[str]) -> dict[str, List[float]]:
        ...
        # yes_token_id = tokenizer(SPACE + candidates[0])["input_ids"][-1]
        # # yes_token_ids = [t[-1] for t in tokenizer.batch_encode_plus([" Yes", "yes", "Yes", " yes"])["input_ids"]]
        # no_token_id = tokenizer(SPACE + candidates[1])["input_ids"][-1]
        # a_token_id = tokenizer(SPACE + candidates[2])["input_ids"][-1]
        # b_token_id = tokenizer(SPACE + candidates[3])["input_ids"][-1]
        #
        # yes_logprobs = []
        # no_logprobs = []
        # a_logprobs = []
