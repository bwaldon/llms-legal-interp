...
base_data_path <- "H:\\Documents\\Github\\llms-legal-interp\\runs\\runs-42_07_16\\Ministral-8B-Instruct-2410-results.csv"
base_data <- read.csv(base_data_path)
instruct_data_path <- "H:\\Documents\\Github\\llms-legal-interp\\runs\\runs-42_07_16\\Ministral-8B-Instruct-2410-results.csv"
...
plot <- ggplot(combined_data, aes(x = prob_diff, y = percent_yes, color = source, shape = source)) +
  ...
  geom_smooth(
    data = filter(combined_data, source == "inst"),
    method = "lm",
    se = FALSE,
    color = "black",
    linetype = "dashed"
...
base_labeled <- get_response_direction(processed_base_data, "base")
inst_labeled <- get_response_direction(processed_inst_data, "inst")
